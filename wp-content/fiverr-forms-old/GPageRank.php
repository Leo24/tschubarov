<?php 
  ...
  $proxies = file(FIFO_PATH.'/proxies.php');
...
class GTB_PageRank implements tbr, pref
{
    ...
    public function getToolbarPageRank($toolbarUrl) {
        ...
        //error_log(date('Y-m-d H:i:s')." to check pr >>>".var_export(array($ret, '===', $toolbarUrl), 1).PHP_EOL, 3, FIFO_PATH.'/pr-check.log');
...
class GTB_Request extends GTB_PageRank
{
    ...
    private static function GetWithCurl($url) {
        ...
        global $proxies; 
        $prx = $proxies[rand(1,1000)];       
        curl_setopt($ch, CURLOPT_PROXY, $prx);
        
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        $str = curl_exec($ch);
        curl_close($ch);
        if (strpos($str, '.google') === false ) {
          error_log(date('Y-m-d H:i:s')." to check pr >>>".var_export(array($str, '===', $url, ' ---', $prx), 1).PHP_EOL, 3, '/var/www/html/youngceaser.com/wp-content/plugins/fiverr-forms/pr-check.log');
        }  
