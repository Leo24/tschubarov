...
function wpcf7_do_something_else($cf7) {
    ...
    if(isset($_POST) && !empty($_POST)){
        ...
        $formData = $_POST;
        $postID = url_to_postid($formData['postURL']);
        if(!empty(get_post_meta($postID, 'ContactFormSeoReports'))){
            delete_post_meta($postID, 'ContactFormSeoReports');
            add_post_meta($postID, 'ContactFormSeoReports', $formData);
        }
    }
