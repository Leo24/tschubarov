...
<?php 
        ...
        $sites = get_field('site_url', $post_id);
        if (!empty($sites)) {
...
-->
		  <?php 
		    ...
		    $rurl = '';
    		$rclass = '';
    		$order_number = get_field('order_number', $post_id);
      		
  		  if ( get_post_meta($post_id, '_generated', true) === 'yes' && file_exists(FIFO_PATH."/reports/".$order_number.".zip") ) {
    		    $rurl = FIFO_URL."/reports/".$order_number.".zip" ;
    		    $rclass = 'active';
    		}
  		   ?>
		  <a href="<?php echo $rurl; ?>" class="dl_rep <?php echo $rclass; ?>">Download report</a>
		</div>
		<?php } ?>
		<!-- @@@ end of fiverr order info -->
        </div>
			</div></div></div>
