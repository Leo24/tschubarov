(function($){
	...
	$(document).ready(function(){
		...
		$('.page_wrap_subscription .tariff_block .itm .tariff_list li').append('<img style="opacity: 0.5" src="/wp-content/themes/betheme/images/plans/icn_question.svg">');

		/*$('#acf-field_56153a3303f70').on('focus', function() {
			$('.acf-form-submit input').prop("disabled",true);
		});

		$('#acf-field_56153a3303f70').on('blur', function() {
			var field = $(this);

			$('.acf-form-submit input').prop("disabled",false);
			$('#order_error_check').remove();

			if (field.val().length > 0) {

				if (!field.val().match(/^[a-z0-9]+$/i)) {
					$('.acf-form-submit input').prop("disabled",true);
					field.after('<div id="order_error_check" style="color: red; font-size: 13px;">Please use only alphanumeric chars here.</div>')
					return;
				}

				var data = {
					'action': 'my_check_order',
					'order_id': field.val()
				};
				...
				jQuery.post('/wp-admin/admin-ajax.php', data, function(response) {
					...
					if (response == 1) {
						$('.acf-form-submit input').prop("disabled",true);
						field.after('<div id="order_error_check" style="color: red; font-size: 13px;">Order with such ID is already exist in Database. Please choose another ID.</div>')
					}
...
//				return false; 
	...
	$('#keywords_textarea').append('<div class="theCount">Keywords used: <span id="linesUsed">0</span><div>');

	$('#keywords_textarea textarea').keydown(function(e) {
	...
	$('#keywords_textarea textarea').on('paste', function () {
		var area = $(this);
		var element = this;
		setTimeout(function () {
			...
			$('#keywords_textarea textarea').val(text).keydown();
			console.log();
		}, 100);
	});

	$(document).on('click', '#features_rating_image', function () {
		$('.ngg-gallery-thumbnail:first a').click();
	});
