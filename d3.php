<?php
/***************************************************************************\
| Dumper    version 1.0.8b                                 |
| (c)2003-2006 zapimir       zapimir@zapimir.net           |
| (c)2005-2006 BINOVATOR     info@.net                                 |
|---------------------------------------------------------------------------|
...
\***************************************************************************/
date_default_timezone_set('Europe/Kiev'); // поставив, щоб встановлювало часовий пояс, інакше скрипт не працює на деяких хостингах
...
$auth = 0;
$error = '';
if (!empty($_POST['server']) && strpos($_POST['server'], ':') !== false) {// щоб можна було вказувати сервер
	$host = $_POST['server'];
} elseif (!empty($_POST['server'])) {
  $host = $_POST['server'].':3306';
} else $host = DBHOST;

if (!empty($_POST['login']) && isset($_POST['pass'])) {
	
	if (@mysql_connect($host, $_POST['login'], $_POST['pass'])){
		setcookie("sxd", base64_encode("SKD101:::{$_POST['login']}:::{$_POST['pass']}:::{$host}"));
...
elseif (!empty($_COOKIE['sxd'])) {
    $credentials = explode(":::", base64_decode($_COOKIE['sxd']));
	if (@mysql_connect($credentials[3], $credentials[1], $credentials[2])){
		$auth = 1;
	}
	else{
		$error = '#' . mysql_errno() . ': ' . mysql_error();
...
class dumper {
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Выход onClick=\"location.href = 'd3.php?reload'\">";
...
<HTML>
<HEAD>
<TITLE> Dumper Lite 1.0.8 | &copy; 2006 zapimir</TITLE>
...
<TR>
<FORM NAME=skb METHOD=POST ACTION=d3.php>
<TD VALIGN=TOP BGCOLOR=#F4F3EE STYLE="padding: 8px 8px;">
{$content}
...
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>Для работы Dumper Lite требуется:<BR> - Internet Explorer 5.5+, Mozilla либо Opera 8+ (<SPAN ID=sie>-</SPAN>)<BR> - включено выполнение JavaScript скриптов (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
...
<TR>
<TD>Сервер:</TD>
<TD><INPUT NAME=server TYPE=text CLASS=text></TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
